 ...
 */
 ...
 // QK_SMEM_STRIDE: Q和K在共享内存中的步幅（一行多长）
   ...
   // 新增：用于记录每个 Kj 的指数化累加和
   float kj_exp_sum[num_tiles_k] = {0.0f}; // 每个线程记录其负责的 Kj 的累加和
   int32_t sparse_count = 0; // 记录被跳过的 Kj 数量
 ...
 // 直白说：查表看这块要干几次活，没活就下班；调整“导航图”（LUT）到当前任务的位置。
   ...
   Sparse_Lut += batch_id * num_qo_heads * num_block_q * num_block_k + head_id * num_block_q * num_block_k + bx * num_block_k;
 ...
 #pragma unroll
     ...
     // 判断是否小于阈值并记录到 Sparse_Lut
     #pragma unroll
     for (uint32_t fk = 0; fk < num_tiles_k; fk++)
     {
       if (kj_exp_sum[fk] < sparse_threshold)
       {
         if (lane_id == 0) // 只用第一个线程写入，避免竞争
         {
           Sparse_Lut[sparse_count] = K_idx_lane_base + fk * MMA_QK_N; // 记录 Kj 的全局位置
           sparse_count++;
         }
       }
     }
       ...
       if (local_max_diff + pv_threshold > 0)
       {
         ...
         for (uint32_t fk = 0; fk < num_tiles_k; fk++)
         {
           ...
           for (uint32_t fq = 0; fq < num_tiles_q; fq++)
           {
             ...
             for (uint32_t k = 0; k < 8; k++)
             {
               float exp_val = expf(RS_f32[fq][fk][k] * sm_scale - m[fq][0]); // 减去最大值以稳定指数计算
               temp_sum += exp_val;
             }
           }
           kj_exp_sum[fk] += temp_sum; // 累加到对应 Kj 的总和
         }
 ...
 #pragma unroll
     ...
     for (uint32_t fk = 0; fk < num_tiles_k; fk++)
     {
       ...
       for (uint32_t fq = 0; fq < num_tiles_q; fq++)
       {
         ...
         for (uint32_t k = 0; k < 8; k++)
         {
           float exp_val = expf(RS_f32[fq][fk][k] * sm_scale - m[fq][0]);
           temp_sum += exp_val;
         }
       }
       kj_exp_sum[fk] += temp_sum;
     }
     ...
     for (uint32_t fk = 0; fk < num_tiles_k; fk++)
     {
       if (kj_exp_sum[fk] < sparse_threshold)
       {
         if (lane_id == 0)
         {
           Sparse_Lut[sparse_count] = K_idx_lane_base + fk * MMA_QK_N;
           sparse_count++;
         }
       }
     }
