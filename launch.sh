...
# sparsity 范围
sparsity_values=$(seq 0.1 0.01 0.1)

echo "将在 ${num_assignments} 个 tmux 窗口/GPU配置 (${gpu_assignments[*]}) 上分配任务..."

# 分配任务
for num_layer in $(seq 10 10); do
    for sparsity in $sparsity_values; do
        ...
        out_path="${BASE_OUT_PATH}/${experiment_name}/_sparsity_${sparsity}_layer_${num_layer}_gpus_${current_gpu_setting//,/}" # 文件路径中使用 GPU 信息 (替换逗号)
        mkdir -p $out_path
